...
## 實體方法 vs @classmethod vs @staticmethod 全景深度剖析

在 Python 的類別設計中，方法主要分為三大流派：**普通實體方法**、**類別方法 (`@classmethod`)** 與 **靜態方法 (`@staticmethod`)**。

理解這三者的關鍵在於：**它們在執行時，Python 到底自動幫你塞進了什麼參數？它們能存取什麼層級的資料？**

---

### 1. 本質、存取權限與感知能力對照

- **普通實體方法 (Instance Method)**：
  - **最標準、最常見的類別方法**。
  - 第一個參數固定為 **`self`（指向具體實體物件）**。
  - **擁有最高的存取權限**：既能讀寫每個物件各自獨立的狀態（`self.name`、`self.hp`），也能存取類別共用屬性。
  - **呼叫限制**：必須先實例化出具體物件才能呼叫。

- **類別方法 (`@classmethod`)**：
  - 第一個參數固定為 **`cls`（指向類別物件本身）**。
  - **感知類別層級狀態**：能讀寫整個類別共用的全域屬性，且能透過 `cls(...)` 動態實例化新物件。
  - **呼叫方式**：直接透過 `類別名.方法名()` 呼叫，無需先建立實體物件。

- **靜態方法 (`@staticmethod`)**：
  - **無任何隱式參數**（既沒有 `self` 也沒有 `cls`）。
  - **完全無感知能力**：它不知道也不關心自己身處在哪個類別或物件裡，純粹進行傳入參數的計算與校驗。
  - **呼叫方式**：直接透過 `類別名.方法名()` 呼叫，無需先建立實體物件。

> **生動比喻單元**：  
> - **普通實體方法 (`self`)**：像「每個員工個人的薪資條與識別證」。必須綁定具體某個員工（物件）才能查詢工時與扣款。  
> - **類別方法 (`cls`)**：像「公司的人資主管」。掌管全體員工的底薪標準、統計在職員工人數，或是負責用標準流程錄取新員工（工廠模式）。  
> - **靜態方法 (`@staticmethod`)**：像「放在公司茶水間牆上的計算機」。誰都可以拿來按 `1 + 1 = 2`，它不關心是哪位員工在按，也不在乎公司目前的總營收。
...
# 情況 A：透過物件實體呼叫
obj = Demo()
obj.instance_func(10)  # 底層轉發為：Demo.instance_func(obj, 10) ➔ 自動塞入 obj 給 self

# 情況 B：透過類別直接呼叫
Demo.class_func(10)     # 底層轉發為：Demo.class_func(Demo, 10) ➔ 自動塞入 Demo 給 cls
Demo.static_func(10)    # 底層轉發為：static_func(10) ➔ 原封不動傳入 10，無任何隱藏參數
```

> **開發避坑：直接用類別呼叫普通實體方法的報錯陷阱**  
> 如果你直接寫 `Demo.instance_func(10)`，Python 會拋出經典錯誤：  
> `TypeError: Demo.instance_func() missing 1 required positional argument: 'self'`  
> 這是因為普通方法**死死依賴具體實體 `self`**，沒有實體物件就無法運作！若不需要 `self`，就應該改用 `@classmethod` 或 `@staticmethod`。

---

### 3. 致命差異：子類別繼承時的表現（工廠模式）

這是區分 `@classmethod` 與其他寫法最重要的核心天條。

##### ❌ 錯誤示範一：使用 @staticmethod 當工廠（繼承破功）
...
在類別中設計方法時，請依照下列決策樹進行選型：

```text
這個方法需要讀取或修改具體物件的專屬屬性（如 self.name）嗎？
│
├─► 需要  ➔ 選擇【普通實體方法】(以 self 為參數)
│          （典型場景：user.save()、player.attack()、car.drive()）
│
└─► 不需要
     │
     └── 需要用到類別本身的資訊或動態實例化嗎？
          │
          ├─► 需要  ➔ 選擇【@classmethod】(以 cls 為參數)
          │          （典型場景：User.from_dict()、Item.get_total_count()）
          │
          └─► 不需要 ➔ 選擇【@staticmethod】(無隱式參數)
                     （典型場景：MathUtils.is_even()、Validator.is_valid_email()）
```

---

### 5. 普通實體方法 vs 類別方法 vs 靜態方法三大金剛全景矩陣

| 比較維度 | 普通實體方法 (Instance Method) | 類別方法 (Class Method) | 靜態方法 (Static Method) |
| :--- | :--- | :--- | :--- |
| **裝飾器標記** | 無（預設常態方法） | **`@classmethod`** | **`@staticmethod`** |
| **第一個隱式參數** | **`self`**（指向當前實體物件） | **`cls`**（指向當前類別物件） | **無**（像普通函式一樣自由傳參） |
| **呼叫前置條件** | **必須先建立實體物件** (`obj = Class()`) | **無需建立實體**，直接 `Class.method()` | **無需建立實體**，直接 `Class.method()` |
