...
| :--- | :--- | :--- | :--- |
| **[[#print() 快速格式化輸出\|rich.print()]]** | 獨立函式 | **免實例化直接色彩輸出與資料結構排版** | 快速除錯、腳本彩色文字列印、字典與清單美化排版 |
| **[[#Console() 建立控制台實例\|Console()]]** | 類別建構子 | **建立 Rich 控制台的核心管理大腦** | 正式 CLI 程式開發、統一控制色彩主題、自定義寬度與目標輸出串流 |
| **[[#print() 控制台輸出\|console.print()]]** | 物件方法 | **輸出豐富文字、標記語法與 UI 渲染組件** | 列印表格、面板、樹狀圖等所有 Rich 組件至控制台 |
| **[[#log() 輸出日誌\|console.log()]]** | 物件方法 | **輸出附帶時間戳記與原始碼行號的結構化日誌** | 程式執行流程追蹤、除錯日誌列印、非阻塞日誌記錄 |
| **[[#status() 加載狀態\|console.status()]]** | 物件方法 | **顯示動態載入動畫轉圈圈 (Spinner)** | 網路請求、資料庫查詢、長時間批次任務的非同步進度提示 |
...
| **[[#Panel() 卡片面板\|Panel()]]** | 類別建構子 | **建立帶有邊框與標題的卡片容器 (預設全寬)** | 重點資訊提示、錯誤警告面板、系統資訊展示 |
| **[[#Panel.fit() 貼合內容卡片面板\|Panel.fit()]]** | 類別方法 | **建立寬度自動貼合文字內容的緊湊卡片面板** | 小型提示徽章、簡短狀態卡片、緊湊對話提示框 |
| **[[#Group() 群組渲染器\|Group()]]** | 類別建構子 | **將多個不同 Rich 組件捆綁為單一可渲染物件** | 解決 Panel 或 Table 儲存格僅能容納單一組件的限制 |
| **[[#Padding() 縮排與內邊距\|Padding()]]** | 類別建構子 | **為渲染組件增加上下左右空白間距** | 控制終端版面留白、階梯式縮排與區塊分離 |
...
| **[[#inspect() 物件探針\|inspect()]]** | 獨立函式 | **全方位掃描並印出 Python 物件的屬性、方法與文檔** | 互動式探索未知套件、快速除錯、物件內部狀態診斷 |
| **[[#install() 全域報錯接管\|traceback.install()]]** | 獨立函式 | **全域接管未捕捉異常並輸出精美彩色 Traceback** | 生產環境除錯、終端異常美化、快速定位程式崩潰行號 |
...
- **使用時機**：
  - 想要建立寬度「剛好包覆內部文字內容（Tight fit）」的自適應邊框面板，而不是撐滿整個終端機螢幕寬度（100% 寬度）。
  - 用於美化短提示訊息、狀態徽章、CLI 對話框或小型卡片，避免過長的多餘空白。
  - 等同於建立 `Panel(renderable, expand=False)` 的官方語意化快捷類別方法。
- **語法**：`panel = Panel.fit(renderable, title=None, subtitle=None, style="none", border_style="none", padding=(0, 1), ...)`
- **參數說明**：
  - `renderable` (`RenderableType`)：要放入面板的內容（純文字字串、或任何 Rich 渲染組件如 `Table`、`Group`）。
  - `title` (`str | Text | None`，選填)：面板頂端邊框中央的標題文字。
  - `subtitle` (`str | Text | None`，選填)：面板底端邊框中央的副標題文字。
  - `border_style` (`StyleType`，選填)：邊框的顏色或樣式（例如 `"cyan"`、`"bold red"`、`"green"`）。
  - `padding` (`PaddingDimensions`，預設 `(0, 1)`)：內部文字與邊框之間的上下與左右內邊距。
- **回傳值**：
  - `Panel`：寬度自動收縮適配內容的面板實例物件。

> **Panel() vs Panel.fit() 關鍵差異**：
> 1. `Panel()` 預設 `expand=True`，面板會自動橫向延展撐滿整個終端機螢幕（100% 寬度）。
> 2. `Panel.fit()` 預設相當於 `expand=False`，面板寬度會自動計算並剛好包覆內容最長的一行加上 padding。
> 3. 若希望在常規 `Panel()` 中達到相同效果，需手動指定 `expand=False`；但使用 `Panel.fit()` 程式碼更加簡潔且語意明確。
...
# 1. Panel.fit() 基礎用法：寬度自動貼合文字，不佔滿整個螢幕
fit_card = Panel.fit("這是寬度剛好的自適應面板！", border_style="green")
console.print(fit_card)

# 2. 帶有標題與副標題的緊湊型面板
status_panel = Panel.fit(
    "[bold yellow]警告：[/bold yellow]伺服器負載較高 (85%)\n請留意系統連線狀態。",
    title="[bold red]系統警報[/bold red]",
    subtitle="[dim]更新於 1 分鐘前[/dim]",
    border_style="red"
)
console.print(status_panel)

# 3. 直觀對比：常規 Panel (預設全寬) vs Panel.fit (貼合寬度)
console.print(Panel("預設 Panel：自動橫向延展撐滿終端機螢幕寬度", title="全寬面板"))
console.print(Panel.fit("Panel.fit：邊框剛好貼齊內容邊界", title="緊湊面板"))
```
